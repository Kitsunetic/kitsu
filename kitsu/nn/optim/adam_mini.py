...
class Adam_mini(torch.optim.Optimizer):
    def __init__(
        ...
        *,
        ...
        self.mlp_names = (
            {}
        )  # Default empty. We will use one learning rate for one layer in MLP block. If not empty, we will use per-neuron vmean for MLPs. Our ablation study shows that using one lr is the same as using per-neuron lr for MLPs.
        ...
        if verbose:
            print(f"Adam-mini found {count_embd} embedding layers, {count_output} output layers, {count_wqk} Querys and Keys.")
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                state = self.state[p]
                if any(embd_name in name for embd_name in self.embd_names) or any(
                    output_name in name for output_name in self.output_names
                ):  # this is for embedding and output layer
                    if p.grad is None:
                        continue
                    if len(state) == 0:
                    ...
                    if len(state) == 0:
                        ...
                        state["vmean"] = torch.zeros_like(
                            state["m"][0 : state["head_per_gpu"], 0:1], memory_format=torch.preserve_format
                        )
                    ...
                    mlp_name in name for mlp_name in self.mlp_names
                ):  # MLP blocks. If True, then Adam-mini will use one lr per output neuron. This will avoid all_reduce when the single MLP block is sharded on different GPUs. However, we find this design will not boost performance. By default, we will not enter here
                    if p.grad is None:
                        continue
                    ...
                    if len(state) == 0:
                        ...
                        state["vmean"] = torch.zeros_like(
                            state["m"][0 : state["neuron_per_gpu"], 0:1], memory_format=torch.preserve_format
                        )
                    ...
                    if len(state) == 0:
                        block_numel = p.new_tensor(p.numel(), dtype=torch.float32)
                        reduced = False
